...
import com.hl.zipkin.jincou.pajiekou.model.JinSePressData;
import com.hl.zipkin.jincou.pajiekou.model.JinSePressResult;
import com.hl.zipkin.jincou.pajiekou.service.CrawlerJinSeLivePressService;
import com.hl.zipkin.jincou.pajiekou.service.LivePressService;
import com.hl.zipkin.jincou.pajiekou.until.CoinPressConst;
import com.hl.zipkin.jincou.pajiekou.until.JsonUtils;
import com.hl.zipkin.jincou.pajiekou.until.OkHttp;
...
public class CrawlerJinSeLivePressServiceImpl extends AbstractCrawlLivePressService implements
    ...
    public void start() {
        try {
            doTask(CoinPressConst.CHAIN_FOR_LIVE_PRESS_DATA_URL_FORMAT);
        } catch (IOException e) {
    ...
    protected List<PageListPress> crawlPage(int pageNum) throws IOException {
        ...
        // 格式化翻页参数
        String requestUrl = String.format(CoinPressConst.CHAIN_FOR_LIVE_PRESS_DATA_URL_FORMAT, PAGE_SIZE, bottomId);
        Response response = OkHttp.singleton().newCall(
                new Request.Builder().url(requestUrl).addHeader("referer", CoinPressConst.CHAIN_FOR_LIVE_URL).get().build())
                .execute();
